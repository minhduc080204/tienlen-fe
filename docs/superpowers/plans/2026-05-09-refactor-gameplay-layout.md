# Refactor Gameplay Layout Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a unified GameLayout component to avoid UI duplication across GamePlay, GamePlayBot, and GamePlayOfflineBot pages.

**Architecture:** Extract common UI structures (Background, Header, Player positions, Table, Bottom Actions) into a reusable `GameLayout` component with sub-components for specific parts like `RoomInfo` and `MyData`.
...
Run: `npm run build` (or check if `tsc` passes)

---

### Task 2: Refactor GamePlay.tsx

**Files:**
- Modify: `src/page/GamePlay.tsx`

- [ ] **Step 1: Update imports and use GameLayout**

Replace the main return statement and use `RoomInfo`, `MyData` and `GameLayout`.

- [ ] **Step 2: Clean up unused code/components**

Remove manual background div and redundant UI blocks.

---

### Task 3: Refactor GamePlayBot.tsx
...
### Task 4: Refactor GamePlayOfflineBot.tsx
