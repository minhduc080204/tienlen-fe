...
export const ProfileTab = ({
  ...
  return (
    <div className="flex flex-col bg-zinc-950 p-4 rounded-lg overflow-y-auto custom-scrollbar h-full w-full">
      {/* Profile Info Header */}
      <div className="flex items-center gap-5 bg-zinc-900 border border-zinc-800/80 p-4 rounded-xl shrink-0">
        <div className="w-20 h-20 rounded-full bg-zinc-800 border-2 border-red-500 flex items-center justify-center text-3xl overflow-hidden shrink-0 shadow-lg shadow-red-950/20">
        ...
        <div>
          ...
          <div className="text-zinc-400 text-sm mb-2">ID: {user.id}</div>
          <div className="flex items-center gap-2 font-semibold">
            <span className="text-yellow-500">💰</span> {formatNumber(user.tokenBalance)} xu
          </div>
        </div>
      </div>

      {/* Select Avatar Sub-tabs */}
      <div className="flex border-b border-zinc-800 gap-4 mt-5 shrink-0">
        <button
          onClick={() => setActiveSubTab("presets")}
          className={`pb-2 text-sm font-semibold transition-colors relative flex items-center gap-2 ${
            activeSubTab === "presets" ? "text-red-500" : "text-zinc-400 hover:text-zinc-200"
          }`}
        >
          <Smile size={16} />
          Mẫu Có Sẵn
        ...
        <button
          ...
          className={`pb-2 text-sm font-semibold transition-colors relative flex items-center gap-2 ${
            activeSubTab === "custom" ? "text-red-500" : "text-zinc-400 hover:text-zinc-200"
          }`}
        >
          <Sparkles size={16} />
          Tự Tạo Avatar
      ...
      {loading ? (
          ...
          <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 shrink-0">
            {styleFilters.map((style) => (
              <button
                ...
                className={`px-3 py-1 text-xs rounded-full border transition font-medium shrink-0 cursor-pointer ${
              ...
              {avatars
                ...
                .map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`relative group bg-zinc-900 border rounded-xl p-3 flex flex-col items-center gap-3 transition-all duration-300 ${
                        isEquipped
                    ...
                    >
                      {/* Avatar preview */}
                      <div className="w-16 h-16 rounded-full bg-zinc-850 p-1 flex items-center justify-center overflow-hidden border border-zinc-800 shadow-inner">
                      ...
                      {!isUnlocked && (
                        <div className="absolute inset-0 bg-black/85 backdrop-blur-[1px] rounded-xl flex flex-col items-center justify-center p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      ...
                      {!isUnlocked && (
                        <div className="absolute top-2 right-2 bg-zinc-950/80 border border-zinc-850 p-1 rounded-full text-zinc-400 group-hover:hidden transition-all">
                      ...
                      {isUnlocked && (
                        <div className="w-full">
                          {isEquipped ? (
                            <div className="w-full bg-green-500/10 border border-green-500/30 text-green-500 text-xs font-bold py-1 px-3 rounded-lg text-center">
                              Đang dùng
                            </div>
                          ) : (
                            <button
                              onClick={() => handleSelectPreset(item)}
                              className="w-full bg-red-600 hover:bg-red-500 active:scale-95 text-white text-xs font-bold py-1 px-3 rounded-lg transition-all cursor-pointer"
                            >
                              Sử dụng
                            </button>
                          )}
        ...
        <div className="flex-1 flex flex-col sm:flex-row gap-4 mt-4 overflow-y-auto custom-scrollbar pb-4 min-h-0">
          {/* Preview card */}
          <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex flex-col items-center justify-center gap-4 min-h-[220px]">
            <div className="text-xs font-semibold text-zinc-400">Xem Trước Avatar</div>
            <div className="w-24 h-24 rounded-full bg-zinc-850 p-2 flex items-center justify-center overflow-hidden border border-zinc-800 shadow-lg shadow-black/40 relative">
              {customPreviewUrl ? (
            ...
            <button
              ...
              className="w-full max-w-[180px] bg-green-600 hover:bg-green-500 disabled:bg-zinc-850 disabled:text-zinc-500 active:scale-95 text-white text-xs font-bold py-2 px-4 rounded-lg transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
            >
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-zinc-900 border border-zinc-800 focus:border-red-500 focus:outline-none rounded-lg p-2.5 text-xs text-zinc-200"
                />
                <button
